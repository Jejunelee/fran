  ...
  return (
    <div className="relative w-full overflow-visible">
      ...
      >
        ...
        {/* Content wrapper */}
        <div className="relative z-10 max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 py-8 md:py-12 min-h-[80vh] md:min-h-[80vh] flex items-center
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[40%_60%] gap-0 w-full">
            ...
            >
              ...
              <motion.div
                ...
                className="mb-6 md:mb-8 lg:mb-10 max-md:mb-4"
              >
                <h2 className="font-serif font-light text-[#750000] leading-[0.6] tracking-[-0.02em] [font-stretch:extra-condensed]
                  max-md:leading-[1]">
                  <span className="block font-serif font-light text-[#750000] text-[clamp(2.2rem,4vw,3.6rem)]
                    max-md:text-[clamp(1.8rem,8.1vw,2.7rem)]">
                    The{" "}
                    <span className="font-script font-medium tracking-[0.02em] text-[clamp(4.2rem,7.5vw,7rem)]
                      max-md:text-[1.85em]">
                      process
                    </span>
                  </span>
              ...
              {/* Process steps */}
              <div className="flex flex-col gap-2 md:gap-2.5 lg:gap-3 max-md:gap-2">
                {processSteps.map((step, index) => (
                  <motion.div
                    key={step.number}
                    variants={itemVariants}
                    custom={index}
                    className="flex items-start gap-3 md:gap-4 lg:gap-5 max-md:gap-3"
                  >
                    <span className="font-josefin font-semibold text-[clamp(1.2rem,1.5vw,1.6rem)] text-[#5a0a0a] leading-[1.3] min-w-[2.4rem] md:min-w-[2.8rem] lg:min-w-[3.2rem] flex-shrink-0 tracking-[0.02em]
                      max-md:min-w-[2.2rem] max-md:leading-[1.2]">
                      {step.number}
                    </span>
                    <div className="flex flex-col max-md:gap-0">
                      <span className="font-josefin font-semibold text-[clamp(1.05rem,1.25vw,1.35rem)] text-[#5a0a0a] leading-[1.3]
                        max-md:leading-[1.2]">
                        {step.title}
                      </span>
                      <span className="font-josefin font-normal text-[clamp(0.9rem,1.05vw,1.15rem)] text-[#5a0a0a] leading-[1.5] mt-0.5
                        max-md:leading-[1.4] max-md:mt-0">
                        {step.description}
                      </span>
                    </div>
                  </motion.div>
